function qualMovimento(a) {
    if (a = "bispo" || "Bispo" || "Bishop" || "bishop")
        console.log("Quantas casas desejar na diagonal e em qualquer direção.");

    else if (a = "Peão" || "peão" || "pawn" || "Pawn")
        console.log("O peão pode em seu primeiro movimento andar duas casas para a frente. Só no primeiro movimento. Posteriormente, ele anda uma casa por vês. Para 'comer' outra peça, o movimento é em sua diagonal, mas só pra frente, nunca pra trás. Há ainda uma regra bem diferenciada chamada 'en passant', ela consiste na ideia de capturar o peão que tentar fugir ao combate.Calma, eu explico; Se um peão estiver por exemplo em d2, sua casa inicial, e houver peão adversario ou em e4 ou em f4, o peão em d2 poderia 'se negar ao combate' uzando seu direito de andar duas casas de uma vez, parando em d4. Mas caso o faça, os peões das casas adjacentes podem caputar este peão sem tocar-lo, parando na casa d3. Daí 'en passant'.");

    else if (a = "Cavalo" || "cavalo" || "Horse" || "horse")
    ...
    else if (a = "rei" || "Rei" || "king" || "King");
    console.log("O rei caminha com sabedoria. Um passo de cada vez, e na direção e sentido que melhor lhe aprouver.");

    else if (a = "Queen" || "queen" || "rainha" || "Rainha");
    console.log("A rainha é sem dúvidas a mais poderosa peça de ataque, e talvez de defesa também. Ela é capaz de andar quantas casas desejar em qualquer direção que desejar. Seu  KI é de 3 milhões.");

    else if (a = "torre" || "Torre" || "Rock" || "rock");
